...
import { SetupShell } from "@/features/evaluator-setup/components/setup-shell";
import { onboardUser } from "@/lib/api/auth/auth-api";
...
export function ProfileSetupForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const password = sessionStorage.getItem("setup_password");
      const token = urlToken || sessionStorage.getItem("setup_token");

      if (!password || !token) {
        throw new Error("Missing password or onboarding token. Please start from the password setup step.");
      }

      const payload = {
        firstName: form.firstName,
        lastName: form.lastName,
        gender: form.gender.toUpperCase(), // Backend expects enum like MALE/FEMALE
      ...
      setError(err.message || "Onboarding failed. Please check your token or try again later.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <form className="space-y-6" onSubmit={handleSubmit}>
        ...
        {error && (
          <div className="p-3 rounded-lg bg-rose-50 border border-rose-200 text-rose-600 text-xs font-medium animate-in fade-in slide-in-from-top-1">
            {error}
          </div>
        )}
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full h-11 bg-[#0A77FF] text-white rounded-lg text-[14px] font-semibold hover:bg-[#0862d1] active:transform active:scale-[0.98] transition-all shadow-[0px_1px_2px_rgba(16,24,40,0.05)] mt-4 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
